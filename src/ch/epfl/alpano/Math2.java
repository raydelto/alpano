...
public interface Math2 {
    ...
    static double firstIntervalContainingRoot(DoubleUnaryOperator f, double minX, double maxX, double dX) {
        ...
        do {
            if (signum(f.applyAsDouble(low)) + signum(f.applyAsDouble(up)) == 0
                    || signum(f.applyAsDouble(low)) * signum(f.applyAsDouble(up)) == 0)
                return low;
            low += dX;
            up += dX;
        } while (up <= maxX);
    ...
    static double improveRoot(DoubleUnaryOperator f, double x1, double x2, double epsilon) {
        ...
        checkArgument(!isSameSign(f.applyAsDouble(x1), f.applyAsDouble(x2)),
                "f(x1) and f(x2) don't have the same sign");
