...
public final class PanoramaUserParameters {
    ...
    public PanoramaUserParameters(Map<UserParameter, Integer> parameters, String name) {
        ...
        int maxH = min(HEIGHT.sanitize(parameters.get(HEIGHT)), (170 * ((WIDTH.sanitize(parameters.get(WIDTH)) - 1)
                / HORIZONTAL_FIELD_OF_VIEW.sanitize(parameters.get(HORIZONTAL_FIELD_OF_VIEW))) + 1));
     ...
     * @param name                   the name to identify the predefined parameters
     */
    public PanoramaUserParameters(int observerLongitude, int observerLatitude, int observerElevation, int centerAzimuth,
            int horizonrtalFieldOfView, int maxDistance, int width, int height, int samplingExponent, int paint,
            String name) {

        this(createMap(observerLongitude, observerLatitude, observerElevation, centerAzimuth, horizonrtalFieldOfView,
                maxDistance, width, height, samplingExponent, paint), name);
     ...
     */
    private static EnumMap<UserParameter, Integer> createMap(int observerLongitude, int observerLatitude,
            int observerElevation, int centerAzimuth, int horizonrtalFieldOfView, int maxDistance, int width,
            int height, int samplingExponent, int paint) {
    ...
    public PanoramaParameters panoramaParameters() {
        ...
        return new PanoramaParameters(
                new GeoPoint(Math.toRadians((double) observerLongitude() / 10000),
                        Math.toRadians((double) observerLatitude() / 10000)),
                observerElevation(), Math.toRadians((double) centerAzimuth()),
                Math.toRadians((double) horizontalFieldOfView()), maxDistance() * 1000, applySuperSampling(width()),
                applySuperSampling(height()));
    }

    /**
     * Creates a new PanoramaParameters without considering the super sampling
     * exponent
     * 
     * @return new PanoramaParameters
     */
    public PanoramaParameters panoramaDisplayParameters() {
        ...
        return new PanoramaParameters(
                new GeoPoint(Math.toRadians((double) observerLongitude() / 10000),
                        Math.toRadians((double) observerLatitude() / 10000)),
                observerElevation(), Math.toRadians((double) centerAzimuth()),
                Math.toRadians((double) horizontalFieldOfView()), maxDistance() * 1000, width(), height());
    }
